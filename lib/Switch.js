...
export class Switch extends Component {
	...
	constructor(props, context) {
	  super(props, context);
		this.transformX = 21;
		if (props.containerStyle && props.containerStyle.width)
      this.transformX = (props.containerStyle.width - 32) / 2;

	  this.state = {
	  	value: props.value,
			transformSwitch: new Animated.Value(
        props.value ? this.transformX : (-1) * this.transformX,
      ),
	...
	animateSwitch(value, cb = () => {}) {
		Animated.parallel([
			Animated.spring(this.state.transformSwitch, {
				toValue: value ? this.transformX : (-1) * this.transformX,
				duration: 200
			}),
